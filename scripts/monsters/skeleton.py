...
class Skeleton:
    def __init__(self, x, y, collision_sprites, players, gravity=0.5, damage=30, attack_range=4000, attack_cooldown=2.0):
        ...
        self.anim_attack_left = [self.one_shot_attack_left, self.two_shot_attack_left, self.three_shot_attack_left, self.four_shot_attack_left]
    ...
    def loading_texture(self):
        ...
        self.one_shot_run = arcade.load_texture("models/monsters/skeleton/animations/run/run1.png")
        self.two_shot_run = arcade.load_texture("models/monsters/skeleton/animations/run/run2.png")
        self.three_shot_run = arcade.load_texture("models/monsters/skeleton/animations/run/run3.png")
        self.four_shot_run = arcade.load_texture("models/monsters/skeleton/animations/run/run4.png")
    ...
    def on_update(self, delta_time):
        ...
        if target_player is not None:
            ...
            if self._last_attack_time >= self.attack_cooldown:
                ...
                self._last_attack_time = 0.0
            self.facing = "right" if target_player.player_sprite.center_x > self.skeleton_sprite.center_x else "left"
        else:
            self.is_attacking = False
            direction = 1 if self.facing == "right" else -1
            next_x = self.skeleton_sprite.center_x + self.speed * direction

            half_width = self.skeleton_sprite.width / 2
            half_height = self.skeleton_sprite.height / 2
            future_left = next_x - half_width
            future_right = next_x + half_width
            future_bottom = self.skeleton_sprite.center_y - half_height
            future_top = self.skeleton_sprite.center_y + half_height
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        if self.texture_change_time_run >= self.texture_change_delay_run:
            ...
            self.skeleton_sprite.texture = tex_list[self.current_texture_run]
